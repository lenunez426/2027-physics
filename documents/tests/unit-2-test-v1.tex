...
\header{{Name:\enspace\makebox[6cm]{\hrulefill}}}{}{Physics FRQ Test on Unit 2: Force Interactions}
% \header{Physics}{Test on Unit 2}{Force Interactions}

% \firstpageheader{\bfseries DO NOT WRITE ON THIS DOCUMENT}{}{Physics Test on Unit 2: Force Interactions}
% \runningheader{\bfseries DO NOT WRITE ON THIS DOCUMENT}{}{Physics Test Unit 2}
...
\begin{tikzpicture}
    ...
    \draw[shift={(1cm,0.5cm)},thick] (0,0) -- (3,0) node[above,pos=0.5] {rope};
...
\question 
A baby elephant stands on the bed. The system includes the elephant, the bed, and the Earth. Which of the following is a correct force pair?

\begin{center}
    \begin{tikzpicture}
        \node at (0,0) {\twemoji[width=2cm]{1f6cf}};
        \draw[shift={(1mm,4.2mm)}] (0,0) node {\twemoji[height=1.7cm]{elephant}};
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}
        \draw (0,0) node {\reflectbox{\twemoji[height=1.5cm]{person in motorized wheelchair}}};
        \draw[thick,<-,shift={(-1.5cm,0)}] (0,0) -- (-3,0) node[above,pos=0.5] {35\,N};  
        \draw[thick,<-,shift={(+1.5cm,0)}] (0,0) -- (+3,0) node[above,pos=0.5] {35\,N};  
    \end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
    ...
    \draw[thick,<-,shift={(1.5cm,0)}] (0,0) -- (1,0) node[above,pos=0.5] {5\,N};
    \draw[thick,<-,shift={(-1.5cm,0)}] (0,0) -- (-1,0) node[above,pos=0.5] {5\,N};
    \draw[thick,<-,shift={(0,1.2cm)}] (0,0) -- (0,2) node[right,pos=0.5] {20\,N};
    \draw[thick,<-,shift={(0,-1.2cm)}] (0,0) -- (0,-2) node[right,pos=0.5] {20\,N};
\end{tikzpicture}
\end{minipage}
\hspace{2mm}
...
\question
A gust of wind pushes a spider dangling from a spider web. At the moment shown below, the spider is momentarily at rest.

\begin{center}
\begin{tikzpicture}
    \draw (0,0) -- (-2,1);
    \draw (0,0) node[rotate=-116] {\twemoji[height=5mm]{spider}};
    \draw[very thick,->] (0.5,0) -- ++(1,0) node[above,pos=0.5] {$F_\mathrm{wind}$};
\end{tikzpicture}
\end{center}

Let the system include the spider, spiderweb, wind, and Earth. Which of the following is NOT a force acting on the spider?

\begin{randomizeoneparchoices}
    \correctchoice normal force
    \choice contact force
    \choice gravitational force
    \choice tension force
\end{randomizeoneparchoices}

\question
Book B is resting on shelf S. 

\begin{center}
\begin{tikzpicture}
    \draw[rotate=45,fill=black!15] (0,0) rectangle ++(1.5,0.5) node[pos=0.5,rotate=45] {B}; 
    \draw[shift={({1.5*cos(45)},0)},fill=black!20] (0,0) rectangle ++(2.2,1.7) node[pos=0.5] {S};
    \begin{scope}[shift={(8,0.8)}]
        \fill (0,0) circle (3pt);
        \draw[thick,->] (0,0) -- ++(-2,0) node[left] {$\vec{F}_{\mathrm{BS}}$};
    \end{scope}
\end{tikzpicture}
\end{center}

The mass of the shelf is greater than the mass of the book. How does the force on the book by the shelf, $F_{\mathrm{BS}}$, compare to the force on the shelf by the book, $F_\mathrm{SB}$?

\begin{randomizechoices}[keeplast]
    \correctchoice $F_\mathrm{BS} = F_\mathrm{SB}$
    \choice $F_\mathrm{BS} > F_\mathrm{SB}$
    \choice $F_\mathrm{BS} < F_\mathrm{SB}$
    \choice It depends on the angle at which the book is inclined. 
\end{randomizechoices}

\question
The figure below shows two force sensors, X and Y, with a rubber hand connecting their hooks. Sensor Y is set in reverse so that a pull to the left is registered as a negative force. 

\begin{center}
\begin{tikzpicture}
    \draw[lightgray,thick,rounded corners=1pt,shift={(1.05,-0.05)}] (0.25,0) rectangle ++(2.4,0.1);
    \node[rotate=-90] at (1.15,0) {\huge \faQuestion};
    \draw[fill=black!8] (-1,-0.6) rectangle ++(2,1.2) node[pos=0.5] {X};
    \begin{scope}[shift={(5,0)}]
        \node[rotate=90] at (-1.15,0) {\huge \faQuestion};
        \draw[fill=black!8] (-1,-0.6) rectangle ++(2,1.2) node[pos=0.5] {Y};
    \end{scope}
\end{tikzpicture}
\end{center}

With the rubber band initially slack, a group of students hold sensor X in place, slowly pull sensor Y to the right while the rubber band becomes taut, then slowly allow sensor Y to return next to sensor X. Which of the following graphs shown the forces detected by each sensor as a function of time?

\hfill \textit{Continued on next page\ldots}

\begin{center}
\begin{tikzpicture}
    \begin{axis}[width=5.5cm,height=6.5cm,
        axis y line=left,
        axis x line=center,
        ylabel={Force (N)},
        xlabel={Time (s)},
        ...
        \addplot[domain=0:5,red,thick,smooth] {5*exp(-(x-2.5)^2)};
        \addplot[domain=0:5,blue,thick,smooth] {-2*exp(-(x-2.5)^2)};
        \legend{X,Y};
        \node[above] at (current bounding box.north) {\bfseries \large Graph A};
    \end{axis}
\end{tikzpicture}
\hspace{2mm}
\begin{tikzpicture}
    \begin{axis}[width=5.5cm,height=6.5cm,
        ...
        \addplot[domain=0:5,red,thick,smooth] {2*exp(-(x-2.5)^2)};
        \addplot[domain=0:5,blue,thick,smooth] {-5*exp(-(x-2.5)^2)};
        \legend{X,Y};
        \node[above] at (current bounding box.north) {\bfseries \large Graph B};
    \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
    \begin{axis}[width=5.5cm,height=6.5cm,
        ...
        \addplot[domain=0:5,blue,thick,smooth] {-5*exp(-(x-2.5)^2)};
        \legend{X,Y};
        \node[above] at (current bounding box.north) {\bfseries \large Graph C};
    \end{axis}
\end{tikzpicture}
\hspace{2mm}
\begin{tikzpicture}
    \begin{axis}[width=5.5cm,height=6.5cm,
        ...
        \addplot[domain=0:5,red,thick,smooth] {5*exp(-(x-2.5)^2)};
        \addplot[domain=0:5,blue,thick,smooth] {-0.1*exp(-(x-2.5)^2)};
        \legend{X,Y};
        \node[above] at (current bounding box.north) {\bfseries \large Graph D};
    \end{axis}
\end{tikzpicture}
\end{center}
...
\ifprintanswers
{\bfseries \color{red} Answer: Graph C}
\fi

\question
Find the net force in the free-body diagram below.

\begin{center}
\begin{tikzpicture}[x=3mm,y=3mm]
    \fill (0,0) circle (4pt);
    \draw[thick,->] (0,0) -- (10,0) node[right] {\SI{10}{N}};
    \draw[thick,->] (0,0) -- (-4,0) node[left] {\SI{4}{N}};
\end{tikzpicture}
\end{center}

\begin{randomizeoneparchoices}
    \correctchoice \SI{6}{N}
    \choice \SI{14}{N}
    \choice \SI{10}{N}
    \choice \SI{40}{N}
\end{randomizeoneparchoices}
...
\begin{randomizeoneparchoices}[norandomize,keeplast]
    \choice speeding up
...
Are the forces on the object balanced or unbalanced?
...
\begin{enumerate} %...These scenarios were generated by ChatGPT.
    ...
    \fillin[gravitational force][5cm]
...
\begin{parts}
\part[2] What is happening to the magnitude of the car's velocity?

\begin{solution}
Give points for any one of the statements below.

\begin{itemize}[topsep=0pt,itemsep=0pt]
    \item The velocity is decreasing in magnitude.
    \item The velocity is changing.
    \item The car is slowing down.
\end{itemize}
\end{solution}
...
\part[1] Are the forces on the car balanced or unbalanced? \fillin[unbalanced][6cm]
\part[2] Draw one of the Multiple Representations of the object's motion.
\end{parts}

\begin{solutionorbox}[6cm]
Any one of the representations below earns credit.
