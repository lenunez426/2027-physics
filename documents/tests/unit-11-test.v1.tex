...
\firstpageheader{DO NOT WRITE ON THIS DOCUMENT\\{Physics}}{}{Practice Test on Unit 11: Conservation of Charge}
...
\subsection*{Test on Unit 11: Conservation of Charge}
...
\question
A positively charged rod touches a neutral metal sphere. The rod becomes less positive and the metal sphere now has an excess of positive charge. The redistribution of charge happens because \fillin[electrons][3cm]\ move from the sphere to the rod.
...
\question
What is the force between two charged spheres, each having a charge of $+\SI{2}{\micro C}$, which are a distance of \SI{3}{m} apart?

\begin{randomizechoices}
    \choice \SI{0.012}{N}
    \choice \SI{2000}{N}
    \correctchoice \SI{0.004}{N}
    \choice \SI{4e9}{N}
\end{randomizechoices}

\question
Two point charges, initially \SI{1}{cm} apart, are moved to a distance of \SI{3}{cm} apart. By what factor does the resulting electric forces between them change?

\begin{randomizechoices}
    \choice The force decrease by $1/3$.
    \correctchoice The force decrease by $1/9$
    \choice The force increase by $3$.
    \choice The force increase by $9$
\end{randomizechoices}
...
% \question
% The magnitude of the electrostatic force between two charged particles is directly proportional to the product of the magnitude of the charges and inversely proportional to the square of the distance between them. This statement best describes\ldots 
...
\question
Your house is wired in\ldots 

\begin{randomizechoices}
    \choice parallel, so that if something is unplugged other components also power off.
    \choice series, so that if something is unplugged other components also power off.
    \correctchoice parallel, so that if something is unplugged other components stay powered.
    \choice series, so that is something is unplugged other components stay powered.
\end{randomizechoices}
...
\question
The figure below shows a portion of an electric circuit.

\begin{center}
\begin{tikzpicture}[y=1.8cm,x=1.5cm]
    \draw (0,0) -- (0,1) -- (1,1) to[R={\SI{4}{\ohm}}] (1,0) -- (0,0);
    \draw (1,1) -- (2,1) to[R={\SI{6}{\ohm}}] (2,0) -- (1,0);
    \draw (2,1) -- (3,1) to[R={\SI{8}{\ohm}}] (3,0) -- (2,0);
    \draw (3,1) -- (4,1) -- (4,0) -- (3,0);
    \draw (-1,0.5) -- (0,0.5) (4,0.5) -- (5,0.5);
\end{tikzpicture}
\end{center}


The equivalent resistance $R_\mathrm{eq}$ of the circuit is
\bigskip
...
\question
In below, if the circuit breaks at Point 1, which light bulb(s) remain on?

\begin{center}
\begin{tikzpicture}[y=2.5cm,x=2.5cm]
    \draw (0,0) to[battery,invert] (0,1) -- (1,1) to[bulb={X}] (1,0) -- (0,0);
    \draw (1,1) -- (2,1) to[bulb={Y}] (2,0) -- (1,0);
    \draw (2,1) -- (3,1) to[bulb={Z}] (3,0) -- (2,0);
    \draw[very thick,fill=white] (2,0.8) circle (2.5pt) node[right=2.5pt] {\small Point 1}; 
\end{tikzpicture}
