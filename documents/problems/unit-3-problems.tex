...
\firstpageheader{{{Name:\enspace\makebox[6cm]{\hrulefill}}}}{}{Physics Unit 3: Changing Motion}
\runningheader{Physics}{Unit 3: Acceleration}{Problems}
...
\subsection*{PhET Simulation: Forces and Motion}

Go to PhET Simulation: Forces and Motion (\href{https://phet.colorado.edu/sims/html/forces-and-motion-basics/latest/forces-and-motion-basics_all.html}{click here}). Click the second panel, called ``Motion.''


\begin{mdframed}
\noindent \textbf{Steps for Resetting the Simulation:}

\vspace{-1ex}

\begin{enumerate}[itemsep=0pt]
    \item Press the orange reset button $\left(\,\circlearrowright\,\right)$.
    \item Check all the boxes:
    \begin{itemize}[itemsep=0pt]
        \item[] \checkbox{1} Force
        \item[] \checkbox{1} Values
        \item[] \checkbox{1} Masses
        \item[] \checkbox{1} Speed
        \item[] \checkbox{1} Stopwatch
    \end{itemize}
    \item Press the pause $\left(\,\text{\faPause}\,\right)$ button once for the \underline{simulation}.
    \item Press the play $\left(\,\text{\faPlay}\,\right)$ button once for the \underline{stopwatch}.
\end{enumerate}

\noindent The steps above ensure the simulation and stopwatch are synchronized so that the stopwatch starts at the same time as the simulation. The simulation is now set for data collection.
\end{mdframed}

\begin{questions}
\question
Follow the ``Steps for Resetting the Simulation'' listed above. 

\begin{parts}

\part
The system in this simulation is the box on the skateboard. What is the mass of the box?

\ifprintanswers
{\color{red} $m = \SI{50}{kg}$} 
\else
\fillwithlines{7mm}
\fi

\part \label{part-a}
The initial speed of the system is the speed when the time on the stopwatch reads \texttt{00:00.00}. What is the initial velocity?
...
\part \label{part-b}
Press the\ \faForward\  button once to add 50 newtons of net force on the system. Run the simulation with \ \faPlay\ and use the step button $\left(\,\rotatebox[origin=c]{-90}{\faEject}\,\right)$ to pause the simulation \underline{exactly} when the stopwatch reads \texttt{00:04.00}. Let the final velocity be the velocity after this 4-second time interval. What is the final velocity?

\ifprintanswers
{\color{red} $v_f = \SI{4.0}{m/s}$}
\else
\fillwithlines{7mm}
\fi

\part
The system's average acceleration is the change in velocity over the change in time:

\begin{equation}
    a = \frac{\Delta v}{\Delta t} = \frac{v_f - v_i}{\Delta t}
\end{equation}

Use the velocities from parts \eqref{part-a} and \eqref{part-b} to calculate the system's acceleration.
...
\part 
Summarize the physical quantities used in this simulation by filling in the table below:

\begin{center}
\def\arraystretch{1.5}
\begin{tabular}{|c|c|c|}
    \hline
    \textbf{Net Force} (N) & \textbf{Mass} (kg) & \textbf{Acceleration} ($\mathrm{m/s^2}$)\\ \hline
     & & \\ \hline
\end{tabular}
\end{center}
...
\textit{When a net force of \fillin[50][2cm]\ newtons is exerted on a box of mass \fillin[50][2cm]\ kilograms,\\[1ex]
the acceleration of the box is \fillin[1.0][2cm]\ $\mathrm{m/s^2}$.}
\end{parts}
...
\begin{parts}
\part 
For each row in the table below, calculate the change in velocity and acceleration of the \SI{50}{kg} box after the given net force $F_\mathrm{net}$ is applied for a 4.0-second time interval. \underline{Note:} After each row, following the ``Steps for Resetting the Simulation'' from the previous page.

\begin{center}
\bgroup
\def\arraystretch{1.5}
% Define a macro for conditional table entries
\newcommand{\answertableentry}[5]{%
  \ifprintanswers
    #1 & #2 & #3 & \textcolor{red}{#4} & \textcolor{red}{#5} \\ \hline
  \else
    #1 & #2 & #3 & \textcolor{white}{#4} & \textcolor{white}{#5} \\ \hline
  \fi
}

\begin{tabular}{|c|c|c|c|c|}
  \hline
  \thead{\textbf{Net Force}, $F_\mathrm{net}$\\(N)} & \thead{\textbf{Mass}, $m$\\(kg)} & \thead{\textbf{Time Interval}, $\Delta t$\\(s)} & \thead{\textbf{Change in Velocity}, $\Delta v$\\(m/s)} & \thead{\textbf{Acceleration}, $a$\\$\left(\mathrm{m/s^2}\right)$}\\ \hline
  \answertableentry{50}{50}{4.0}{4.0}{1.0}
  \answertableentry{100}{50}{4.0}{8.0}{2.0}
  \answertableentry{150}{50}{4.0}{0}{}
  \answertableentry{200}{50}{4.0}{0}{0}
  % \answertableentry{250}{50}{4.0}{0}{0}
  \answertableentry{300}{50}{4.0}{0}{0}
  \answertableentry{400}{50}{4.0}{0}{0}
  \answertableentry{500}{50}{4.0}{40.0}{10.0}
\end{tabular}
\egroup
\end{center}
...
\part
Make a graph of net force vs.\,acceleration using the data in the table above.
...
\begin{tikzpicture}
    \begin{axis}[height=6cm,
        ...
        axis lines=left,
        ylabel={Net Force (N)},
        xlabel={Acceleration $\left(\mathrm{m/s^2}\right)$},
        ymin=0,ymax=500,
        xmin=0,xmax=10,
        ytick={0,100,...,500},
        xtick={0,1,...,10},
        grid=both,
        minor y tick num=1,
        title={Net Force vs. Acceleration of a 50-kg box}
    ]
        \ifprintanswers
        \addplot[red,very thick,mark=*] coordinates{(1,50)(2,100)(3,150)(4,200)(6,300)(8,400)(10,500)};
        \fi
    \end{axis}
\end{tikzpicture}
\end{center}
...
\question
Select any new mass other than \SI{50}{kg}. Apply 5 different net forces and calculate that system's acceleration. Fill in the table and graph below. Repeat the ``Steps for Resetting the Simulation'' as needed.
...
\newcommand{\answertableentry}[3]{%
  \ifprintanswers
    \textcolor{red}{#1} & \textcolor{red}{#2} & \textcolor{red}{#3} \\ \hline
  \else
    \textcolor{white}{#1} & \textcolor{white}{#2} & \textcolor{white}{#3} \\ \hline
  \fi
}

\begin{tabular}{|c|c|c|c|c|}
  \hline
  \thead{\textbf{Net Force}, $F_\mathrm{net}$\\(N)} & \thead{\textbf{Mass}, $m$\\(kg)} & \thead{\textbf{Acceleration}, $a$\\$\left(\mathrm{m/s^2}\right)$}\\ \hline
...
\question
Using patterns from your tables and graphs, synthesize an equation for net force $F_{\mathrm{net}}$ of an object in terms of the object's mass $m$ and acceleration $a$. The equation should start $F_\mathrm{net} = $ \hfill .
...
\begin{questions}
\question
An \SI{0.080}{kg} egg is dropped from rest and reaches a velocity of \SI{4.4}{m/s}. Calculate the egg's change in momentum.
...
\begin{center}
    \begin{tikzpicture}
        \begin{scope}[rotate=-10,transform shape]
            ...
            \draw[shift={(0,9mm)}] (8,0) rectangle ++(1.5,-0.7) node[pos=0.5,shift={(0,1mm)}] {cart};
            \draw[fill=white,shift={(0,2mm)}] (8.4,0) circle (2mm);
            \draw[fill=white,shift={(0,2mm)}] (9.1,0) circle (2mm);
            \draw[ultra thick,->,shift={(0,6mm)}] (8,0) -- ++(-2,0);
        \end{scope}
    \end{tikzpicture}
\end{center}
...
\subsection*{Net Force Equals Mass Times Acceleration}
