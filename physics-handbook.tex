...
\begin{tikzpicture}[x=7mm,y=7mm]
    \draw[step=7mm,gray] (0,0) grid (10,3);
...
\begin{tikzpicture}[x=7mm,y=7mm]
    \draw[step=7mm,gray] (0,0) grid (10,3);
    \draw[very thick,->] (0,2) -- ++(4,0) node[above,pos=0.5] {$\vec{A}$};
...
\textbf{\Gls{velocity1}} ($v$) is \glsdesc{velocity1}, or the rate at which an object changes position. \textbf{\Gls{average velocity}} ($\bar{v}$) is \glsdesc{average velocity}. Mathematically, 
...
A \textbf{\gls{velocity vs. time graph}} is \glsdesc{velocity vs. time graph}. Beside velocity information, velocity vs. time graphs contain implicit position and displacement data.

\begin{example}
Make a velocity vs. time graph corresponding to the position vs. time graph below.
...
\begin{tikzpicture}
    ...
    \addplot[very thick,domain=0:5] {2*x};
    \end{axis}
\end{tikzpicture}
\end{center}
\end{example}
...
\begin{tikzpicture}
    ...
    \addplot[very thick,domain=0:10] plot {10};
\end{axis}
\end{tikzpicture}
\end{center}
\end{example}
...
\textbf{\Gls{mass}} is \glsdesc{mass} (kg). 

\begin{center}
\begin{minipage}{6cm}
\centering
\begin{tikzpicture}
    \fill (0,0) circle (3pt) node[right=3pt] {$m_\text{\tiny H} = \SI{1.67e-27}{kg}$};
    \node[below=1ex] at (current bounding box.south) {1 Hydrogen atom};  
\end{tikzpicture}
\end{minipage}%
\begin{minipage}{5cm}
\centering
\begin{tikzpicture}
  ...
  % Clip to the square so points stay inside
  \begin{scope}
    \clip (-1,-1) rectangle ++(2,2);
    % Randomly scatter 100 points inside the square
    \foreach \i in {1,...,100}{
      \fill (rand, rand) circle (1.5pt);
    }
  \end{scope}
  \node[below=1mm] at (current bounding box.south) {Many Hydrogen atoms}; 
  \node[above=1mm] at (current bounding box.north) {Gas cloud}; 
\end{tikzpicture}
\end{minipage}
\end{center}

For example, in the figure above, the mass of the gas cloud is the sum of the total mass of all its constituent Hydrogen atoms. 

\begin{example}
    Suppose the gas cloud in the figure above contains 6000 moles of hydrogen. By Avogadro's number, 1 mole equals \num{6.02e23} atoms. Find the mass of the gas cloud.
\end{example}

\textit{Solution:}

The number of hydrogen atoms that constitute the gas cloud is

\begin{align*}
    N &= \SI{6000}{mol} \\[1ex]
    &= \SI{6000}{mol} \times \frac{\SI{6.02e23}{atoms}}{\SI{1}{mol}} \\[1ex]
    &= \SI{3.612e27}{atoms}
\end{align*}
...
\begin{equation*}
    m = N m_\text{\tiny H} = \left(\SI{3.612e27}{atoms}\right) \left(\SI{1.67e-27}{kg}\right) = \boxed{\SI{6.0}{kg}}
\end{equation*}

\hfill $\blacksquare$

\textbf{\Gls{inertia}} is \glsdesc{inertia}. 

Inertia is proportional to mass: the more mass an object has, the greater its inertia. 
...
\begin{tikzpicture}
    ...
    \addplot[very thick,domain=0:10] plot coordinates {(0,0)(4,4)(10,-4)};
\end{axis}
\end{tikzpicture}
\end{center}
...
\textbf{\Gls{kinetic energy}} ($K$) is \glsdesc{kinetic energy}. Mathematically, it's defined as
...
\textit{Solution:}
In base SI units, mass is $m = \SI{0.250}{kg}$.

\begin{equation*}
    K = \frac{1}{2} mv^2 = \frac{1}{2}(\SI{0.25}{kg})(\SI{3.2}{m/s})^2 = \boxed{\SI{1.28}{J}} 
\end{equation*}
...
In the figure below, Alice is in frame A standing at rest on the street. She observes Bob, who is in Frame B, moving at speed $v_\text{\tiny BA}$ relative to her frame. A pelican $P$ moves relative to both frames.

\begin{center}%...Borrowed from Halliday & Resnick 4-6 Relative Motion
\begin{tikzpicture}[x=3cm,y=3cm]
    \draw (0,0) -- (0,1.2) node[above] {$y$} node[above=5mm] {Frame A};
    \draw (0,0) -- (3.5,0) node[right] {$x$};

    %...Alice
    \fill[lightgray] (0,0) circle (2pt) node[left=2pt,black] {0} node[above right=-2.5pt,black] {\Strichmaxerl[2]} node[black,shift={(0.23,0.15)}] {\scriptsize Alice};
    \draw (1,0) node {\tiny $|$};
    \draw (3,0) node {\tiny $|$};
    \draw[|-|,thick] (0,-0.1) -- (1,-0.1) node[below,pos=0.5] {$x_\text{\tiny BA}$};
    \draw[|-|,thick] (0,-0.3) -- (3,-0.3) node[below,pos=0.5] {$x_\text{\tiny PA}$};

    %...Bob
    \begin{scope}[shift={(1,0.3)}]
        \draw (0,0) -- (0,0.9) node[above] {$y$} node[above=5mm] {Frame B};
        \draw (0,0) -- (3,0) node[right] {$x$}; 
        \draw[ultra thick,->] (0,0) -- (1,0) node[above] {$\vec{v}_\text{\tiny BA}$};
        \fill[lightgray] (0,0) circle (2pt) node[above right=-2.5pt,black] {\Strichmaxerl[2]} node[black,shift={(0.23,0.15)}] {\scriptsize Bob};
        \draw (2,0) node[above=-4pt] {\tiny $|$};
        % \draw[fill=lightgray] (2,0.75) circle (1.5mm) node[right=1.5mm] {$P$};
        \draw (2,0.75) node {\fcBirdE{0.02}{black}{0.5}} node[right=5mm] {Pelican};
        \draw[|-|,thick] (0,-0.1) -- (2,-0.1) node[below,pos=0.5] {$x_\text{\tiny PB}$};
    \end{scope}
\end{tikzpicture}
\end{center}
...
\begin{tabular}{|c|l|}
    \hline
    \thead{\bfseries Variable} & \thead{\bfseries What it means} \\ \hline
    $x_\text{\tiny BA}$ & position of Bob relative to Alice \\ \hline
    $x_\text{\tiny PA}$ & position of the Pelican relative to Alice \\ \hline
    $x_\text{\tiny PB}$ & position of the Pelican relative to Bob \\ \hline
    $v_\text{\tiny BA}$ & velocity of Bob as measured from Alice's reference frame \\ \hline
\end{tabular}
\end{center}

Alice and Bob disagree on the Pelican's position. According to Bob, its position is $x_\text{\tiny PB}$. However, according to Alice, it's

\begin{equation*}
    x_\text{\tiny PA} = x_\text{\tiny BA} + x_\text{\tiny PB}
\end{equation*}

Similarly, Alice and Bob disagree on the Pelican's velocity. Because velocity is the rate of change in position, the relative velocities follow the same form as above:

\begin{equation} \label{77nBP}
    v_\text{\tiny PA} = v_\text{\tiny BA} + v_\text{\tiny PB}
\end{equation}
...
\begin{tabular}{|c|l|}
    \hline
    \thead{\bfseries Variable} & \thead{\bfseries What it means} \\ \hline
    $v_\text{\tiny PA}$ & velocity of the Pelican relative to Alice \\ \hline
    $v_\text{\tiny BA}$ & velocity of Bob relative to Alice \\ \hline
    $v_\text{\tiny PB}$ & velocity of the Pelican relative to Bob \\ \hline
\end{tabular}
\end{center}


\begin{example}
    If Bob, from the figure above, moves at \SI{32}{m/s} away from Alice, and Alice claims the Pelican is moving at \SI{45}{m/s} in the $-x$-direction, what is the Pelican's velocity relative to Bob?
\end{example}

\textit{Solution:}
Equation \eqref{77nBP} for relative velocity is
...
\begin{equation*}
    \SI{-45}{m/s} = \SI{32}{m/s} + v_\text{\tiny PB}
\end{equation*}

Solving for the Pelican's velocity from Bob's frame results in

\begin{equation*}
    v_\text{\tiny PB} = \SI{-45}{m/s} - \SI{32}{m/s} =  \boxed{\SI{-77}{m/s}}
\end{equation*}
...
In this unit, there are six forces you should know: the gravitational force, the normal force, the frictional force, the applied force, the spring force, and the tension force.
...
Where does the force $F_\mathrm{g}$ come from? It comes from Earth, which gravitationally pulls the ball down. The gravitational force will be explored in more detail in Unit 5: Force Analysis.
...
The \textbf{\gls{friction}} ($F_f$) is \glsdesc{friction}. An \textbf{\gls{applied force}} ($F_\mathrm{A}$) is \glsdesc{applied force}. In the example below, if a box is resting on a rough surface and an applied force moves it to the right with constant motion, then there is a frictional force in the opposite direction.
...
Another example involving tension is a game of tug-of-war, shown below.
...
A \textbf{\gls{free-body diagram}} is \glsdesc{free-body diagram}. They are a tool for analyzing net force and predicting motion. A free body diagram is focused on one object only and shows a moment in time, as shown below.
...
The free-body diagram includes the forces acting \textit{on} the object but no forces \textit{by} the object. It consists of a dot with arrows (force vectors) coming off the object for every force interaction, as shown below.

\begin{example}
    Name all the forces and their directions shown in the free-body diagram above.
\end{example}

\textit{Solution:}

The free-body diagram includes the normal force ($F_\mathrm{N}$) pointing up, the applied force ($F$) pointing right, the gravitational force ($F_g$) pointing down, and the frictional force ($F_f$) pointing left. 

\hfill $\blacksquare$

On a flat surface, the normal force $F_\mathrm{N}$ equals the gravity force $F_g$ in magnitude. But $F_\mathrm{N}$ is affected by inclining the surface on which the object rests, as shown below.
...
\begin{tikzpicture}[x=5mm,y=5mm]
    ...
    \draw[->] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->,thick] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
\end{tikzpicture}
\end{minipage}%
\begin{minipage}{3cm}
\centering
\begin{tikzpicture}[x=5mm,y=5mm,rotate=-20]
    \draw (-2.5,-1) -- (2.5,-1);
    \fill (0,0) circle (2pt);
    \draw[thick] (-1,-1) rectangle (1,1);
    \draw[->,thick] (0,0) -- (0,{3*cos(20)}) node[above] {$F_\mathrm{N}$};
    \draw[rotate around={20:(2.5,-1)}] (-2.5,-1) node[above,shift={(5mm,0)}] {\ang{20}} -- (2.5,-1);
    \draw[->,rotate=20,] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->,opacity=0,rotate=20] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
\end{tikzpicture}
\end{minipage}%
\hspace{2mm}
\begin{minipage}{3cm}
...
\begin{tikzpicture}[x=5mm,y=5mm,rotate=-40]
    ...
    \draw[->,opacity=0,rotate=40] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
\end{tikzpicture}
\end{minipage}%
\hspace{2mm}
\begin{minipage}{3cm}
\centering
\begin{tikzpicture}[x=5mm,y=5mm,rotate=-60]
    \draw (-2.5,-1) -- (2.5,-1);
    \fill (0,0) circle (2pt);
    \draw[thick] (-1,-1) rectangle (1,1);
    \draw[->,thick] (0,0) -- (0,{3*cos(60)}) node[right] {$F_\mathrm{N}$};
    \draw[rotate around={60:(2.5,-1)}] (-1,-1) node[above,shift={(5mm,0)}] {\ang{60}} -- (2.5,-1);
    \draw[->,rotate=60] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->,opacity=0,rotate=60] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
\end{tikzpicture}
\end{minipage}%
\hspace{2mm}
\begin{minipage}{3cm}
\centering
\begin{tikzpicture}[x=5mm,y=5mm,rotate=-75]
    \draw (-2.5,-1) -- (2.5,-1);
    \fill (0,0) circle (2pt);
    \draw[thick] (-1,-1) rectangle (1,1);
    \draw[->,thick] (0,0) -- (0,{3*cos(75)}) node[right=2pt] {$F_\mathrm{N}$};
    \draw[rotate around={75:(2.5,-1)}] (-0.5,-1) node[above,shift={(5mm,0)}] {\ang{75}} -- (2.5,-1);
    \draw[->,rotate=75] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->,opacity=0,rotate=75] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
\end{tikzpicture}
\end{minipage}
\caption{The normal force on a box changes as the angle of incline changes.}
\label{Zmibb}
\end{figure}

\begin{example}
According to Figure \ref{Zmibb}, how does the magnitude and direction of the normal force vector $F_\mathrm{N}$ change as the surface becomes more inclined?
\end{example}

\textit{Solution:}
The magnitude of the normal force decreases as the angle of incline decreases. The direction of the normal changes relative to the observer but remains perpendicular to the surface.

\hfill $\blacksquare$

For a free-body diagram on an inclined surface, it is useful to break the gravity force vector $F_g$ in two mutually perpendicular components: one that is parallel to the surface, and one that is perpendicular to the surface. 

Let $F_{g,\parallel}$ be the parallel component of the gravity force $F_g$, and $F_{g,\perp}$ be the perpendicular component. These components of gravity, for the cases displayed in Figure \ref{Zmibb}, are shown below:
...
\centering
\begin{tikzpicture}[x=5mm,y=5mm]
    \draw[opacity=0.2] (-2.5,-1) -- (2.5,-1);
    \fill (0,0) circle (2pt);
    \draw[thick] (-1,-1) rectangle (1,1);
    \draw[->] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
\end{tikzpicture}
\end{minipage}%
\begin{minipage}{3cm}
\centering
\begin{tikzpicture}[x=5mm,y=5mm,rotate=-20]
    \draw[opacity=0.2] (-2.5,-1) -- (2.5,-1);
    \draw[opacity=0.2,rotate around={20:(2.5,-1)}] (-2.5,-1) -- (2.5,-1);
    \fill (0,0) circle (2pt);
    \draw[thick] (-1,-1) rectangle (1,1);
    \draw[->] (0,0) -- (0,{3*cos(20)}) node[above] {$F_\mathrm{N}$};
    \draw[->,rotate=20,opacity=0.1] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->,opacity=0,rotate=20] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
    \draw[->] (0,0) -- ({3*sin(20)},0) node[right] {$F_{g,\parallel}$};
    \draw[->] (0,0) -- (0,{-3*cos(20)}) node[below] {$F_{g,\perp}$};
\end{tikzpicture}
\end{minipage}%
\hspace{2mm}
\begin{minipage}{3cm}
...
\begin{tikzpicture}[x=5mm,y=5mm,rotate=-40]
    \draw[opacity=0.2] (-2.5,-1) -- (2.5,-1);
    \draw[opacity=0.2,rotate around={40:(2.5,-1)}] (-1,-1) -- (2.5,-1);
    \fill (0,0) circle (2pt);
    \draw[thick] (-1,-1) rectangle (1,1);
    \draw[->] (0,0) -- (0,{3*cos(40)}) node[above] {$F_\mathrm{N}$};
    \draw[->,rotate=40,opacity=0.1] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->,opacity=0,rotate=40] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
    \draw[->] (0,0) -- ({3*sin(40)},0) node[right] {$F_{g,\parallel}$};
    \draw[->] (0,0) -- (0,{-3*cos(40)}) node[below] {$F_{g,\perp}$};
\end{tikzpicture}
\end{minipage}%
\hspace{2mm}
\begin{minipage}{3cm}
...
\begin{tikzpicture}[x=5mm,y=5mm,rotate=-60]
    \draw[opacity=0.2] (-2.5,-1) -- (2.5,-1);
    \draw[opacity=0.2,rotate around={60:(2.5,-1)}] (-1,-1) -- (2.5,-1);
    \fill (0,0) circle (2pt);
    \draw[thick] (-1,-1) rectangle (1,1);
    \draw[->] (0,0) -- (0,{3*cos(60)}) node[right] {$F_\mathrm{N}$};
    \draw[->,rotate=60,opacity=0.1] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->,opacity=0,rotate=60] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
    \draw[->] (0,0) -- ({3*sin(60)},0) node[below] {$F_{g,\parallel}$};
    \draw[->] (0,0) -- (0,{-3*cos(60)}) node[below] {$F_{g,\perp}$};
\end{tikzpicture}
\end{minipage}%
\hspace{2mm}
\begin{minipage}{3cm}
...
\begin{tikzpicture}[x=5mm,y=5mm,rotate=-75]
    \draw[opacity=0.2] (-2.5,-1) -- (2.5,-1);
    \draw[opacity=0.2,rotate around={75:(2.5,-1)}] (-0.5,-1) -- (2.5,-1);
    \fill (0,0) circle (2pt);
    \draw[thick] (-1,-1) rectangle (1,1);
    \draw[->] (0,0) -- (0,{3*cos(75)}) node[right=2pt] {$F_\mathrm{N}$};
    \draw[->,rotate=75,opacity=0.1] (0,0) -- (0,-3) node[below] {$F_g$};
    \draw[->,opacity=0,rotate=75] (0,0) -- (0,+3) node[above] {$F_\mathrm{N}$};
    \draw[->] (0,0) -- ({3*sin(75)},0) node[below] {$F_{g,\parallel}$};
    \draw[->] (0,0) -- (0,{-3*cos(75)}) node[left=2pt] {$F_{g,\perp}$};
\end{tikzpicture}
\end{minipage}%
\caption{The components $F_{g,\parallel}$ and $F_{g,\perp}$ of the gravity force change as the angle incline changes.}
\label{txqna}
\end{figure}

\begin{example}
In Figure \ref{txqna}, how do the parallel and perpendicular components of the gravitational force on the box change as the surface becomes more inclined?
\end{example}

\textit{Solution:}
As the angle of incline increases, the parallel component $F_{g,\parallel}$ increases in magnitude, and the perpendicular component $F_{g,\perp}$ decreases in magnitude.

\hfill $\blacksquare$

\begin{example}
According to Figure \ref{txqna}, what appears to be the relationship between the normal force and the perpendicular component of gravity?
\end{example}

\textit{Solution:}
The figure shows that $F_\mathrm{N}$ are $F_{g,\perp}$ are always equal in magnitude and opposite and direction.

\hfill $\blacksquare$


\subsection{Newton's Third Law and Interacting Force Pairs}
\textbf{\gls{Newton's third law of motion}} states that \glsdesc{Newton's third law of motion}. ``For every action, there is an equal but opposite reaction.'' This law represents a certain symmetry in nature: Forces always occur in pairs, and one body cannot exert a force on another without experiencing a force itself.
%This third law, sometimes known as the action-reaction law, is summarized 

For example, in 2022 Astros pitcher Phil Maton exerted a large force by punching a locker, and the locker instantaneously returned a force of equal magnitude, resulting in Maton fracturing a bone in his right hand. There was a force pair interaction between Maton's hand and the locker.

\begin{center}
    \includegraphics[width=5cm]{documents/figures/maton.jpeg}
\end{center}

The following are rules for interacting force pairs:

\begin{itemize}[itemsep=0pt,topsep=0pt]
    \item An object can be involved in multiple force interactions.
    \item Any one interaction is between two objects and involves two forces.
    \item Forces always come in pairs.
    \item Each force in a force pair is the same magnitude (size or strength) but acts in opposite directions.
\end{itemize}

\bigskip

\begin{example}
A basketball rests on a table, and the table rests on the Earth. Identify the third-law force pairs and non-force-pairs in this scenario.
...
\begin{tikzpicture}
    ...
    \draw (0,1.85) node {\twemoji[width=1cm]{basketball}} node[above=7mm] {Basketball $B$};
...
Force pairs occur only between two interacting objects.

\begin{center}
\begin{minipage}{4cm}
\centering
\begin{tikzpicture}
    \draw (0,0) node[opacity=0.5] {\twemoji[width=2cm]{globe showing Americas}};
    \draw[very thick,->] (0,0) -- (0,1.5) node[right] {$\vec{F}_{EB}$};
    \draw (0,4) node[opacity=0.5] {\twemoji[width=1cm]{basketball}};
    \draw[very thick,->] (0,4) -- ++(0,-1.5) node[right] {$\vec{F}_{BE}$};
    \node[above] at (current bounding box.north) {Force Pair};
\end{tikzpicture}  
\end{minipage}%
\hspace{2mm}
\begin{minipage}{4cm}
\centering
\begin{tikzpicture}
    \draw[pattern=north west lines] (-1.6,0) -- (1.6,0) -- (1.6,0.15) -- (-1.6,0.15) -- cycle;
    \draw (0,0.65) node[opacity=0.5] {\twemoji[width=1cm]{basketball}};
    \draw[very thick,->] (0,0.65) -- ++(0,1.5) node[right] {$\vec{F}_{BT}$};
    \draw[very thick,->] (0,0) -- ++(0,-1.5) node[right] {$\vec{F}_{TB}$};
    \node[above] at (current bounding box.north) {Force Pair};
\end{tikzpicture}
\end{minipage}%
\hspace{2mm}
...
\centering
\begin{tikzpicture}
    \draw[white] (-1.6,0) -- (1.6,0) -- (1.6,0.15) -- (-1.6,0.15) -- cycle;
    \draw (0,0.65) node[opacity=0.5] {\twemoji[width=1cm]{basketball}};
    \draw[very thick,->] (0,0.65) -- ++(0,1.5) node[right] {$\vec{F}_{BT}$};
    \draw[very thick,->] (0,0.65) -- ++(0,-1.5) node[right] {$\vec{F}_{BE}$};
    \node[above] at (current bounding box.north) {NOT Force Pair};
\end{tikzpicture}
\end{minipage}%
\end{center}
...
\section{Changing Motion I: Acceleration}
...
\textbf{\Gls{net force}} ($F_\text{net}$) is related to \glsdesc{net force}. Mathematically, it can be related to acceleration as

\begin{equation*}
    F_\text{net} = ma
\end{equation*}

Often, it's pronounced ``F equals M A.''
...
Momentum is $p = mv$. Momentum changes if there is a change in velocity, $\Delta v$. Change in momentum is given by $\Delta p = p_f - p_i$ and by

\begin{align}
    \Delta p &= m \Delta v \label{TuKpz} \\[1ex]
    &= m \left(v_f - v_i\right) \notag
\end{align}

\begin{example} \label{tbjCT}
The velocity of a \SI{15}{kg} object changes according to the function $v(t) = \left(\SI{0.5}{m/s^2}\right)t + \SI{2}{m/s}$. What is the change in momentum during the entire time shown in the graph below?
...
\begin{tikzpicture}
    \begin{axis}[height=4.5cm,width=5.5cm,
        ...
        \addplot[very thick, domain=0:8] {2+0.5*x};
    \end{axis}
\end{tikzpicture}
\end{center}
\end{example}
...
According to the graph, the initial and final velocities are $v_i = \SI{20}{m/s}$ and $v_f = \SI{60}{m/s}$. By Equation \eqref{TuKpz},

\begin{align*}
    \Delta p &= m \Delta v \\[1ex]
    &= m (v_f - v_i) \\[1ex]
    &= (\SI{15}{kg})(\SI{6}{m/s} - \SI{2}{m/s}) \\[1ex]
    &= \boxed{\SI{60}{kg\cdot m/s}}
\end{align*}

\subsection{Change in Kinetic Energy}

Kinetic energy is $K = \frac{1}{2} mv^2$. Kinetic energy changes if velocity changes. Change in kinetic energy is given by $\Delta K = K_f - K_i$ and by

\begin{align} \label{h5Vlx}
    \Delta K &= \frac{1}{2}m v_f^2 - \frac{1}{2} m v_i^2 \\[1ex]
    &= \frac{1}{2} m \left(v_f^2 - v_i^2\right) \notag
\end{align}

\begin{example}
The graph representing the motion of the same \SI{15}{kg} object from Example \ref{tbjCT} is repeated below.
...
Now calculate the change in kinetic energy of the object.
\end{example}

\textit{Solution:}

By Equation \eqref{h5Vlx},

\begin{align*}
    \Delta K &= \frac{1}{2} m v_f^2 - \frac{1}{2} m v_i^2 \\[1ex]
    &= \frac{1}{2}(\SI{15}{kg})\left(\SI{6}{m/s}\right)^2 - \frac{1}{2}(\SI{15}{kg})\left(\SI{2}{m/s}\right)^2 \\[1ex]
    &= \boxed{\SI{240}{J}}
\end{align*}
...
\section{Changing Motion II: Impulse and Work}
...
\begin{center}
    \begin{tikzpicture}
    \begin{scope}[shift={(-0.5,0)},yshift=-2mm]
        \draw[dashed] (0,0) rectangle (5mm,5mm);
    \end{scope}
    \begin{scope}[shift={(6,0)},yshift=-2mm]
        \draw[fill=black!20] (0,0) rectangle (5mm,5mm);
    \end{scope}
    \draw[ultra thick, ->] (0,0) -- (2,0) node[above=2pt] {$F$};
    \draw[->, dashed] (0,-0.5) -- ++(6,0) node[below,pos=0.5] {$d$};
    \end{tikzpicture}
\end{center}

\textit{Solution:} 

We are given the quantities of force and distance: $F = \SI{72.0}{N}$ and $d = \SI{25.0}{m}$. The unknown quantity we want to find is work.
...
\begin{tikzpicture}[rotate=10]
    \begin{scope}[shift={(0,0.18)}]
        \draw[|<->|] (0,0) -- (8,0) node[above,pos=0.5] {$d$};
    \end{scope}
    \draw (0,0) circle (5mm) node[above=5mm] {$m_1$};
    \draw (8,0) circle (8mm) node[above=8mm] {$m_2$};
    \fill (0,0) circle (2pt);
    \fill (8,0) circle (2pt);
    \draw[thick,->,right=1mm] (0,0) -- (2,0) node[below,pos=0.5] {$F_g$};
    \draw[thick,->,left=1mm] (8,0) -- (6,0) node[below,pos=0.5] {$F_g$};
\end{tikzpicture}
\end{center}
...
In Figure 1, by Newton's law of gravitational, the gravitational force is

\begin{equation*}
    F_1 = \frac{GmM}{D^2}
\end{equation*}

Let $F_2$ be the force in Figure 2, when the distance has doubled. Then,

\begin{align*}
    F_2 &= \frac{G m M}{(2D)^2} \\[1.5ex]
    &= \frac{GmM}{4D^2} \\[1.5ex]
    &= \frac{1}{4} \cdot \frac{GmM}{D^2} \\[1.5ex]
    &= \frac{1}{4} F_1
\end{align*}

So, $F_2$ is 4 times weaker than $F_1$. That is, in Figure 2, the gravitational force decreases by a factor of 4 compared to Figure 1, when the distance is doubled. \hfill $\blacksquare$


\subsection{Mass and Weight}

Recall that \textbf{\gls{mass}} is \glsdesc{mass}. It does not change based on location. In equations it is represented as $m$.

\textbf{\Gls{weight}} is \glsdesc{weight}. 

Using a force sensor, you can plot the weight (force of gravity) as a function of mass, using a variety of masses, as shown below.

\begin{center}
\begin{minipage}[c][5cm][c]{0.3\textwidth}
\centering
\begin{tikzpicture}
    \draw[decorate,decoration={coil,segment length=3pt,post length=2mm,pre length=0mm,amplitude=3mm}] (0,0) node[above] {force sensor} -- ++(0,-2);
...
The slope of this graph is about \SI{10}{N/kg} and represents the strength of the gravitational field near the surface of the Earth. Therefore, weight is defined mathematically as

\begin{equation}
    F_g = mg
\end{equation}

where $g = \SI{10}{N/kg}$ is the gravitational field strength. Being a force, weight is measured in newtons. It changes depending on location. 

Below is a table comparing and contrasting weight and mass.
...
\def\arraystretch{1.5}
    \begin{tabular}{|l|l|}
        ...
        variable: $F_g$ & variable: $m$ \\ \hline
        vector (magnitude and direction) & scalar (magnitude only) \\ \hline
        e.g., $F_g = \SI{120}{N}$ down & e.g., $m = \SI{5}{kg}$ \\ \hline
        subject to change & does not change (constant) \\ \hline
    \end{tabular}
\end{center}
...
\begin{equation*}
    F_g = mg = (\SI{12}{kg})(\SI{10}{m/s^2}) = \boxed{\SI{120}{N}}    
\end{equation*}
...
The \textbf{\gls{friction}} ($F_f$) is \glsdesc{friction}. Friction is parallel to the surface.
...
\subsection{Analyzing Forces using Vectors}
...
The \textbf{\gls{kinematic equations}} are \glsdesc{kinematic equations}. They may be derived from the work-energy theorem and impulse-momentum theorem as follows.

\begin{example}
    Use the work-energy theorem to derive a kinematic equation.
\end{example}
...
Solving this equation for the term $v_f^2$ results in one of the kinematic equations:

\begin{equation*}
    \boxed{v_f^2 = v_i^2 + 2 a \Delta x}
\end{equation*}

Note that this equation does not depend on mass.

\hfill $\blacksquare$

\begin{example}
    Use the impulse-momentum theorem to derive a kinematic equation.
\end{example}
...
Solving this equation for final velocity leads to the kinematic equation

\begin{equation*}
    \boxed{v_f = v_i + at}
\end{equation*}

Note that this is a re-arrangement of the law of acceleration,
...
The two kinematic equations derived in the examples above may be combined to synthesize the four equations for uniformly accelerated motion. These equations are summarized below:

\begin{align}
    v_f &= v_i + a t \label{Ty1d0} \\[1ex]
    \Delta x &= v_i t + \frac{1}{2}a t^2 \label{bxVcq} \\[1ex]
    v_f^2 &= v_i^2 + 2 a \Delta x \label{UOYTP} \\[1ex]
    \Delta x &= \frac{1}{2} \left(v_i + v_f\right) \label{vGg8T} t
\end{align}

where $v_f$ is final velocity, $v_i$ is initial velocity, $a$ is acceleration, $\Delta t$ is the time interval during which the acceleration occurred, and $\Delta x$ is displacement. All units are in meters, seconds, meters per second, or $\mathrm{m/s^2}$.

\subsection{Accelerated Horizontal Motion}

\begin{example} %...Taken from PhysicsClassroom.com
    A car coasts into a hill at \SI{17.0}{m/s}. It slows down with a uniform acceleration of \SI{-1.0}{m/s^2}. (a) What is the car's speed after \SI{5.0}{s}? (b) What is the car's displacement after \SI{5.0}{s}? (c) What is the car's speed after \SI{10.0}{s}? (d) What is the car's displacement after \SI{10.0}{s}?
\end{example}

\textit{Solution:}
First, we list the known variables, identify the unknown, and evaluate which equation is needed to solve the problem. 

(a) The known and unknown variables are:

\begin{align*}
    v_i &= \SI{17}{m/s} \\[1ex]
    a &= \SI{-1.0}{m/s^2} \\[1ex]
    \Delta t &= \SI{5.0}{s} \\[1ex]
    v_f &=\: \text{?}
\end{align*}

These are related by Equation \eqref{Ty1d0}, which gives the answer:

\begin{align*}
    v_f &= v_i + a \Delta t \\[1ex]
    &= \SI{17}{m/s} + \left(\SI{-1.0}{m/s^2}\right)(\SI{5.0}{s}) \\[1ex]
    &= \boxed{\SI{12}{m/s}}
\end{align*}

(b) By Equation \eqref{bxVcq}, the displacement is

\begin{align*}
    \Delta x &= v_i \Delta t + \frac{1}{2}a \Delta t^2 \\[1ex]
    &= (\SI{17}{m/s}) (\SI{5.0}{s}) + \frac{1}{2} \left(\SI{-1.0}{m/s^2}\right)(\SI{5.0}{s})^2 \\[1ex]
    &= \boxed{\SI{72.5}{m}}
\end{align*}

(c) If now $\Delta t = \SI{10}{s}$, then the velocity is

\begin{align*}
    v_f &= v_i + a \Delta t \\[1ex]
    &= \SI{17}{m/s} + \left(\SI{-1.0}{m/s^2}\right)(\SI{10.0}{s}) \\[1ex]
    &= \boxed{\SI{7}{m/s}}
\end{align*}

(d) And with $\Delta t = \SI{10}{s}$, displacement is now

\begin{align*}
    \Delta x &= v_i \Delta t + \frac{1}{2}a \Delta t^2 \\[1ex]
    &= (\SI{17}{m/s}) (\SI{10}{s}) + \frac{1}{2} \left(\SI{-1.0}{m/s^2}\right)(\SI{10}{s})^2 \\[1ex]
    &= \boxed{\SI{120}{m}}
\end{align*}

\hfill $\blacksquare$

\subsection{Free-fall: Accelerated Vertical Motion}

\textbf{\Gls{free fall}} is \glsdesc{free fall}. By setting $a = -g$, the kinematic equations for one-dimensional free fall motion are
...
\begin{center}
\begin{tikzpicture}[x=3.5mm,y=3.5mm,declare function={t(\x)=\x/(18*cos(45));}]
    %...Draw red trajectory:
    \draw[red,domain=0:40,thick,smooth] plot(\x,{trajectoryequation(\x,18,45)+10});
    ...
    \pgfplotsinvokeforeach{0,5,10,21,26,31,36,40}{
            ...
            \draw[densely dashed,->] (#1,{trajectoryequation(#1,18,45)+10}) -- ++(0,{\vectorsize*(18*sin(45) - 9.8*t(#1))});
            \draw[densely dashed,->] (#1,{trajectoryequation(#1,18,45)+10}) -- ++({\vectorsize*18*cos(45)},0);
            \draw[thick,->] (#1,{trajectoryequation(#1,18,45)+10}) -- ++({\vectorsize*18*cos(45)},{0.3*(18*sin(45) - 9.8*t(#1))});
            }

    %...For the apex:
    \draw[fill=black] (16.51,{trajectoryequation(16.51,18,45)+10}) circle (2pt);
    \draw[thick,->] (16.51,{trajectoryequation(16.51,18,45)+10}) -- ++({\vectorsize*18*cos(45)},0);

    %...For the initial point:
    \draw (0,{trajectoryequation(0,18,45)+10}) ++ (0,{\vectorsize*(18*sin(45))}) node[above] {$v_{iy}$};
    \draw (0,{trajectoryequation(0,18,45)+10}) ++ ({\vectorsize*(18*cos(45))},0) node[right] {$v_{ix}$};
    \draw (0,{trajectoryequation(0,18,45)+10}) ++ ({\vectorsize*(18*cos(45))},{\vectorsize*(18*sin(45))}) node[above left] {$v_i$};

    %...For the intermediate point:
    \node[left] at (31,9) {$v_y$};
    \node at (34,13) {$v_x$};
    \node at (34.5,10) {$v$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[x=2cm,y=2cm]
    ...
    \begin{scope}[shift={({cos(45)},{sin(45)})},rotate around={45:(0,0)}]
        \draw[thick,->] (0,0) -- (0,1) node[left] {$v_0$};
        \draw[thick,->] (0,0) -- (-0.8,0) node[above left,pos=0.8] {$a_0$};
        \fill (0,0) circle (3pt);
    \end{scope}
\end{tikzpicture}
\end{center}
...
\subsection{Gravitational and Elastic Potential Energy}
...
In an object-Earth system, when the object is raised from the surface of the Earth to a height $h$, there is a gain in gravitational potential energy.
...
\begin{tikzpicture}
    ...
    \draw[densely dashed] (-2,3) node[left,align=center] {\small zero\\line} -- (2,3);
...
A zero line is a reference point from which changes in gravitational potential energy can be measured. A displacement $\Delta y$ above the zero line increases the gravitational potential energy; a negative displacement decreases the energy:
...
\begin{tikzpicture}
    ...
    \end{scope}
    \draw (3,0) arc (0:180:3);
    \draw[densely dashed] (-2,4) node[left,align=center] {\small zero\\ \small line} -- (2,4);
    \draw[<->] (0,0) -- (0,3) node[right,pos=0.5] {$R$};
    \draw[<->] (0,3) -- (0,4) node[right,pos=0.5] {$y_0$};
...
\begin{tikzpicture}
    ...
    \end{scope}
    \draw (3,0) arc (0:180:3);
    \draw[densely dashed] (-2,5) -- (2,5) node[right,align=center] {\small zero\\ \small line};
    \draw[<->] (0,0) -- (0,3) node[right,pos=0.5] {$R$};
...
If the zero line is near the surface, no more than about \SI{10}{km} above the surface, the change in gravitational potential energy is given by
...
\begin{equation} \label{QyLUh5}
    \mathrm{ME} = K + \mathrm{PE} = \mathrm{constant}
\end{equation}
...
If we replace each $K$ and $\mathrm{PE}$ with $\frac{1}{2}mv^2$ and $mgh$, respectively, the law of conservation of energy becomes
...
An object with a mass of \SI{0.50}{kg} moving at \SI{1.00}{m/s} to the right experiences an elastic collision with an object of mass \SI{1.50}{kg} moving at \SI{0.50}{m/s} to the left.

\begin{center}
\begin{tikzpicture}[x=3cm,y=3cm]
    \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{0.50}{kg}};
    \draw[->,very thick] (0,0) -- (1.0,0) node[above,pos=0.8] {$\left|v\right| = \SI{1.00}{m/s}$};
    \begin{scope}[shift={(3,0)}]
        \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{1.50}{kg}};
        \draw[->,very thick] (0,0) -- (-0.5,0) node[above,pos=1.4] {$\left|v\right| = \SI{0.50}{m/s}$};
    \end{scope}
    \node[above=2mm] at (current bounding box.north) {\bfseries Before Elastic Collision};
\end{tikzpicture}
...
\begin{tikzpicture}[x=3cm,y=3cm]
    \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{0.50}{kg}};
    \draw[->,very thick] (0,0) -- (-1.25,0) node[above,pos=0.8] {$\left|v\right| = \SI{1.25}{m/s}$};
    \begin{scope}[shift={(1,0)}]
        \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{1.50}{kg}};
        \draw[->,very thick] (0,0) -- (0.25,0);
    \end{scope}
    \node[above=2mm] at (current bounding box.north) {\bfseries After Elastic Collision};
\end{tikzpicture}
\end{center}

 If the first object's velocity after collision is \SI{-1.25}{m/s}, what is the second object's velocity?
\end{example}

\textit{Solution:}

We are given the following masses and velocities:

\begin{align*}
    m_1 &= \SI{0.50}{kg} \\[1ex]
    v_{1i} &= \SI{1.00}{m/s} \\[1ex]
    m_2 &= \SI{1.50}{kg} \\[1ex]
    v_{2i} &= \SI{-0.50}{m/s}
\end{align*}

We want the final velocity of the second mass:

\begin{equation*}
    v_{2f} = \ ?
\end{equation*}

By conservation of momentum for a hit-and-bounce (elastic) collision, 
...
\begin{equation*}
    m_2 v_{2f} = m_1 v_{1i} + m_2 v_{2i} - m_1 v_{1f} 
\end{equation*}

Diving both sides by mass $m_2$ leads to
...
So, it recoils at \SI{0.25}{m/s} to the right.

\hfill $\blacksquare$


\begin{example} \label{usQeew}
A \SI{0.50}{kg} object that is moving at a velocity of \SI{1.00}{m/s} collides with a \SI{1.50}{kg} object that is moving at \SI{-0.500}{m/s}. The collision is perfectly inelastic (hit-and-stick). What is the final velocity of the combined system after the collision?
...
\begin{tikzpicture}[x=3cm,y=3cm]
    ...
    \begin{scope}[shift={(3,0)}]
        ...
        \draw[->,very thick] (0,0) -- (-0.5,0) node[above,pos=1.4] {$\left|v\right| = \SI{0.50}{m/s}$};
    \end{scope}
    \node[above=2mm] at (current bounding box.north) {\bfseries Before Inelastic Collision};
\end{tikzpicture}
...
\begin{tikzpicture}[x=3cm,y=3cm]
    \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{0.50}{kg}};
    \draw[->,very thick] (0,0) -- (-0.25,0);
    \begin{scope}[shift={(0.54,0)}]
        \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{1.50}{kg}};
    \end{scope}
    \node[above=2mm] at (current bounding box.north) {\bfseries After Inelastic Collision};
\end{tikzpicture}
\end{center}
...
We want the final velocity combined system:

\begin{equation*}
    v_{f} = \ ?
\end{equation*}

By conservation of momentum for a hit-and-stick (perfectly inelastic) collision, 

\begin{equation*}
    m_1 v_{1i} + m_2 v_{2i} = (m_1 + m_2) v_f
\end{equation*}

Diving both sides by mass $m_1 + m_2$ gives

\begin{align*}
    v_f &= \frac{m_1 v_{1i} + m_2 v_{2i}}{m_1 + m_2} \\[1ex]
    &= \frac{(\SI{0.50}{kg})(\SI{1.0}{m/s}) + (\SI{1.50}{kg})(\SI{-0.50}{m/s})}{\SI{0.50}{kg} + \SI{1.50}{kg}} \\[1ex]
    &= \boxed{\SI{-0.125}{m/s}}
\end{align*}

So the system moves at \SI{0.125}{m/s} to the left.
...
\begin{center}
\begin{tikzpicture}[x=3cm,y=3cm]
    \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{70}{kg}};
    \begin{scope}[shift={(0.54,0)}]
        \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{5.0}{kg}};
    \end{scope}
    \node[above=2mm] at (current bounding box.north) {\bfseries Before Explosion};
\end{tikzpicture}

\vspace{5mm}

\begin{tikzpicture}[x=3cm,y=3cm]
    \draw[opacity=0] (0,0) -- (-2,0);
    \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{70}{kg}};
    \draw[->,very thick] (0,0) -- (-0.5,0);% node[above,pos=0.8] {$\left|v\right| = \SI{1.00}{m/s}$};
    \begin{scope}[shift={(1,0)}]
        \draw[fill=black!10] (0,0) circle (8mm) node[above] {\SI{5.0}{kg}};
        \draw[->,very thick] (0,0) -- (2,0) node[above,pos=0.5] {$\left|v\right| = \SI{50}{m/s}$};
    \end{scope}
    \node[above=2mm] at (current bounding box.north) {\bfseries After Explosion};
\end{tikzpicture}
\end{center}
\end{example} 

\textit{Solution:} 
These are the known and unknown quantities:
...
For an explosion, conservation of momentum states

\begin{equation*}
    0 = m_1 v_{1f} + m_2 v_{2f}
\end{equation*}

Subtracting $m_2 v_{2f}$ from both sides yields

\begin{equation*}
    m_1 v_{1f} = -m_2 v_{2f}
\end{equation*}

Diving both sides by $m_1$ leads to

\begin{equation*}
    v_{1f} = -\frac{m_2 v_{2f}}{m_1} = -\frac{(\SI{5}{kg})(\SI{50}{m/s})}{\SI{70}{kg}} 
    = \boxed{-\SI{3.57}{m/s}}
\end{equation*}

So, Nancy recoils at \SI{3.57}{m/s} in the opposite direction as the potato sack.
...
To \textbf{\gls{oscillate}} means \glsdesc{oscillate}. \textbf{\Gls{oscillatory motion}} is \glsdesc{oscillatory motion}. 


An object attached to the end of a \href{https://ophysics.com/w1.html}{spring} experiences oscillatory motion when the system is displaced from equilibrium. The \textbf{\gls{equilibrium position}} is \glsdesc{equilibrium position}.

\begin{center}
\begin{tikzpicture}[y=1.2cm]
    %...wall, equilibrium line, and amplitude
    \draw[fill=black!15] (-0.4,1.5) -- (0,1.5) -- (0,-1.5) -- (-0.4,-1.5);
    \draw[thick,densely dashed] (3,-2.2) -- (3,2.2) node[above,align=center] {equilibrium\\position};
    \draw[<->] (3.1,-1.7) -- (6,-1.7) node[below,pos=0.5] {amplitude};
    
    %...springs:
    \draw[decorate,decoration={coil,segment length=1.5mm,post length=2mm,pre length=1mm,amplitude=3mm}] (0,1) -- ++(3,0) node[right=3mm] {no force};
    \draw[decorate,decoration={coil,segment length=0.75mm,post length=2mm,pre length=1mm,amplitude=3mm}] (0,0) -- ++(1.5,0);
    \draw[decorate,decoration={coil,segment length=3mm,post length=2mm,pre length=1mm,amplitude=3mm}] (0,-1) -- ++(6,0);

    %...attached objects
    \draw[fill=black!20] (2.85,0.85) rectangle ++(0.3,0.3);
    \draw[fill=black!20] (1.35,-0.15) rectangle ++(0.3,0.3);
    \draw[fill=black!20] (5.85,-1.15) rectangle ++(0.3,0.3);

    %...forces
    \draw[thick,<-] (1.7,0) -- ++(0.6,0) node[right] {$F$} node[right=1cm] {compression};
    \draw[thick,->] (6.2,-1) -- ++(0.6,0) node[right] {$F$ \hspace{2mm} stretch}; 
\end{tikzpicture}
\end{center}

The \textbf{\gls{amplitude}} is \glsdesc{amplitude}.
%A \textbf{\gls{medium}} is \glsdesc{medium}. 

The \textbf{\gls{restoring force}} is \glsdesc{restoring force}. In a spring, the restoring force acting against the applied force is the spring force.

\begin{center}
\begin{tikzpicture}[x=3mm,y=3mm]
    \draw[black!20,decorate,decoration={coil,segment length=1.5mm,post length=2mm,pre length=1mm,amplitude=3mm}] (-1,0) -- ++(-10,0);
    \draw[->,thick] (0,0) -- (4,0) node[right] {$F_\mathrm{spring}$};
    \draw[->,thick] (0,0) -- (-4,0)node[left] {$F_\mathrm{applied}$};
    \draw[fill=black!20] (-1,-1) rectangle (1,1);
    \node[above=2mm] at (current bounding box.north) {compression};
\end{tikzpicture}
\hspace{1cm}
\begin{tikzpicture}[x=3mm,y=3mm]
    \draw[black!20,decorate,decoration={coil,segment length=3mm,post length=2mm,pre length=1mm,amplitude=3mm}] (-1,0) -- ++(-10,0);
    \draw[->,thick] (0,0) -- (4,0) node[right] {$F_\mathrm{applied}$};
    \draw[->,thick] (0,0) -- (-4,0)node[left] {$F_\mathrm{spring}$};
    \draw[fill=black!20] (-1,-1) rectangle (1,1);
    \node[above=2mm] at (current bounding box.north) {stretch};
\end{tikzpicture}
\end{center}

In the spring systems shown above, the restoring force is given by Hooke's law:

\begin{equation*}
    \vec{F}_s = - k \Delta \vec{x}
\end{equation*}

A \textbf{\gls{pendulum}} is \glsdesc{pendulum}.

\begin{center}
\begin{tikzpicture}[x=4cm,y=4cm]
    \fill[lightgray] (-0.3,0) rectangle (0.3,0.03);
    \coordinate (P) at (0,-1);
    \draw (0,0) -- (P) node[pos=0.5,above right] {$l$};
    \fill (P) circle (0.025);
\end{tikzpicture}
\hspace{2cm}
\begin{tikzpicture}[x=4cm,y=4cm]
...
The pendulum's restoring force has magnitude $mg \sin{\theta}$:

\begin{center}
%%%...Tikzpicture inspired by Boas (Mathematical Physics) pg. 344)
\begin{tikzpicture}[x=5cm,y=5cm]
    \draw[lightgray] (0,0) -- (0,-1.3) node[below,black,align=center] {\small equilibrium\\ \small position};
    \fill[lightgray] (-0.3,0) rectangle (0.3,0.03);
    \coordinate (P) at ({sin(30)},{-cos(30)});
    \draw (0,0) -- (P) node[pos=0.45,above right] {$l$};
    ...
    \begin{scope}[shift={(P)},x=2cm,y=2cm,rotate=30]
        \draw[very thick,->] (P) -- (0,{1.3*cos(30)}) node[pos=0.8,right=0.05] {$F_\text{\tiny T}$};
...
\begin{tikzpicture}[x=5cm,y=5cm]
    ...
    \begin{scope}[shift={(P)},x=2cm,y=2cm,rotate=30]
        ...
        \fill (0,0) circle (0.07) node[right=0.07] {$m$};
        \draw[-{Latex[length=2mm,width=2mm]},very thick] (0,0) -- ++(0,{-1.3*cos(30)}) node[right=2mm,pos=0.7] {$mg \cos{\theta}$}; 
        \draw[densely dashed, rotate=-30] (0,0) -- (0,-1.3);
...
Recall that kinetic energy is 

\begin{equation}
    E_k = \frac{1}{2} mv^2
\end{equation}

Gravitational potential energy relative to a zero line is
...
\begin{equation}
    E_s = \frac{1}{2} k \left(\Delta x\right)^2
\end{equation}

where $k$ is the spring constant, and $\Delta x$ is displacement from equilibrium.
...
Draw the energy bar chart for this exact moment.
\end{example}

\textit{Solution:}

Only spring potential energy is present:
...
\begin{axis}[
    ...
    ymin=0,
    ymax=90,
    symbolic x coords={$E_k$,$E_g$,$E_s$,$E_\mathrm{total}$},
    xtick=data,
    yticklabel=\empty
]
...
Only gravitational potential energy is present:
...
Springs and pendulums have periods and frequencies of oscillation. The \textbf{\gls{period}} is \glsdesc{period}, and is measured in seconds. The \textbf{\gls{frequency}} is \glsdesc{frequency}. The period is generally

\begin{equation}
    T = \text{period} = \frac{\text{amount of time}}{\text{number of cycles}}
\end{equation}

Frequency is

\begin{equation}
    f = \text{frequency} = \frac{\text{number of cycles}}{\text{amount of time}}
\end{equation}

The unit of period is the second (s). The unit of frequency is the hertz (Hz) defined as 1 over seconds:

\begin{equation*}
    \SI{1}{hertz} = \SI{1}{Hz} = \frac{1}{1\,\text{second}}
\end{equation*}
...
\begin{example}
    A mass oscillating on a spring completes 1.0 cycle every 0.25 seconds. What is the frequency of the wave?
\end{example}
...
\begin{equation*}
    f = \frac{1}{T} = \frac{1}{\SI{0.25}{s}} = \SI{4}{\Hz}\ .
\end{equation*}
...
The period and frequency of springs and pendulums may be calculated from their physical characteristics. The period ($T_s$) of a spring is affected by the mass of the object and the spring constant of the spring and is given by

\begin{equation}
    T_s = 2\pi \sqrt{\frac{m}{k}}
\end{equation}

where $m$ is the object's mass and $k$ is the spring constant.

The period ($T_p$) of a pendulum is affected by the length of the string and gravitational acceleration, and is given by 

\begin{equation}
    T_p = 2\pi \sqrt{\frac{l}{g}}
\end{equation}

where $l$ is the length of the string and $g = \SI{10}{m/s^2}$ is gravitational acceleration.



\subsection{Amplitude, Frequency, and Period in Simple Harmonic Motion (SHM)}
...
\subsection{Boundary Reflections}

If the boundary is a \textit{fixed} end, the reflected wave \textit{flips} direction. If the boundary is a \textit{loose} end, the reflected wave \textit{looks} the same.

\begin{itemize}
    \item fixed $\rightarrow$ flips
    \item loose $\rightarrow$ looks the same
\end{itemize}
...
\subsection*{Factor of Change Method with Coulomb's Law}

The structure of Coulomb's law is as follows:

\begin{equation*}
    \text{electric force} = \frac{\left(\text{Coulomb's constant}\right) \times \left(\text{charge 1}\right) \times \left(\text{charge 2}\right)}{\left(\text{distance}\right)^2}
\end{equation*}

When charges are made to charge 1, charge 2, or distance, the electric force will change.


\begin{mdframed}
\centering

\textbf{The 3-step Factor of Change Method}

\begin{itemize}%[label=\textbf{Step \arabic*:}]
    \item \textbf{Step 1:} Ask yourself, ``What is changing?'' Assign a number (a factor) to that change.\\(e.g., ``Doubled'' = 2, ``Tripled'' = 3, ``Halved'' = 1/2).
    \item \textbf{Step 2:} Plug only those simple factors into the Coulomb’s Law structure to find your overall multiplier.\\Ignore $k$ and anything that stays the same (just treat them as a 1).
    \item \textbf{Step 3:} To get the new force, multiply your original force by this new multiplier.
\end{itemize}
\end{mdframed}
...
What is changing? Charge 2 is doubled, so $q_1$ increases by a factor of 2.

The multiplier is

\begin{equation*}
    \text{multiplier} = \frac{1\cdot 2 \cdot 1}{1^2} = 2
\end{equation*}

The new force is

\begin{align*}
    \text{new force} &= \text{old force} \cdot \text{multiplier} \\[1ex]
    &= \SI{320}{N} \times 2 \\[1ex]
    &= \boxed{\SI{640}{N}}
\end{align*}

Therefore, if one of the charges doubles, the force also doubles. \hfill $\blacksquare$
...
What is changing? The distance is doubled (from \SI{3}{cm} to \SI{6}{cm}), so $r$ increases by a factor of 2.

The multiplier is

\begin{equation*}
    \text{multiplier} = \frac{1\cdot 1 \cdot 1}{2^2} = \frac{1}{4}
\end{equation*}
...
\begin{align*}
    \text{new force} &= \text{old force} \cdot \text{multiplier} \\[1ex]
    &= \SI{320}{N} \times \frac{1}{3} \\[1ex]
    &= \boxed{\SI{80}{N}}
\end{align*}

Therefore, if the distance doubles, the force decreases to one-fourth of the old value. \hfill $\blacksquare$
...
The negative sign reflects the fact that electrons carry the negative charge. So, although the electrons flow from the cloud to the ground, the positive current is defined to flow from the ground to the cloud.

\hfill $\blacksquare$

An \textbf{\gls{electric circuit}} is a \glsdesc{electric circuit}. A \textbf{\gls{circuit component}} is \glsdesc{circuit component}. Examples of circuit components include wires, batteries, light bulbs, resistors, switches, and fuses.

\textbf{\Gls{voltage2}} is \glsdesc{voltage2}.

A \textbf{\gls{resistor}} is a \glsdesc{resistor}. \textbf{\Gls{resistance}} is \glsdesc{resistance}.


An analogy: Current is like the number of cars passing through a school zone in a given time (e.g. 10 cars per minute), and resistance is like the flashing yellow lights that force the cars to slow down to 25 MPH.
...
\begin{center}
\begin{tikzpicture}
    \draw (0,0) to[bulb] (1,0) node[right] {bulb};
    \draw[shift={(3,0)}] (0,0) to[battery1, voltage=american, v={~}] (1,0) node[right] {battery};
    \draw[shift={(6.5,0)}] (0,0) to[cute open switch] (2,0) node[right]{switch (open)};
    \draw[shift={(12,0)}] (0,0) to[cute closed switch] (2,0) node[right]{switch (open)};
\end{tikzpicture}

\begin{tikzpicture}
    \draw (0,0)  to[resistor] (2,0) node[right] {resistor};
    \draw[shift={(4.5,0)}] (0,0) to[rmeter,t=A] (2,0) node[right] {ammeter};
    \draw[shift={(9,0)}] (0,0) to[rmeter,t=V] (2,0) node[right=1mm]{voltmeter};
    \draw[shift={(13.5,0)}] (0,0) -- (1.5,0) node[right] {wire};
\end{tikzpicture}
\end{center}

A \textbf{\gls{simple circuit}} is \glsdesc{simple circuit}. The following figure shows a circuit diagram of a simple circuit.

\begin{center}
\begin{tikzpicture}[x=2.5cm,y=2.5cm]
    \draw (0,0) to[battery1,invert,l={$V$},i_>={$I$}] (0,1) -- (1,1) to[R={$R$},i_>={$I$}]  (1,0) -- (0,0);
\end{tikzpicture}
\end{center}

\textbf{\gls{Ohm's law}} states that \glsdesc{Ohm's law}. Mathematically, Ohm's law is

\begin{equation*}
    I = \frac{V}{R}
\end{equation*}

To manipulate Ohm's law algebraically, use the Ohm's law triangle:
...
This triangle is useful for remembering the following formulas:

\begin{equation*}
    V = IR \qquad \qquad
    I = \frac{V}{R} \qquad \qquad
    R = \frac{V}{I}
\end{equation*}

\begin{example}
Use Ohm's law to calculate the current in the circuit below.

\begin{center}
\begin{tikzpicture}[x=2cm,y=1.5cm]
    \draw (0,0) -- (0,1) to[battery1,invert,l={\SI{4.5}{V}}] (1,1) -- (1,0) to[R={\SI{500}{\ohm}}] (0,0);
\end{tikzpicture}
\end{center}
\end{example}
...
\begin{equation*}
    I = \frac{V}{R} = \frac{\SI{4.5}{V}}{\SI{500}{\ohm}} = \boxed{\SI{0.009}{A}}
\end{equation*}

This current may be expressed as 9 milliamperes (\SI{9}{mA}). \hfill $\blacksquare$


\subsection{Circuits in Series and in Parallel}
...
\addcontentsline{toc}{section}{Multiple Representations in the Physics Classroom}
...
\addcontentsline{toc}{section}{Equation Sheet}
